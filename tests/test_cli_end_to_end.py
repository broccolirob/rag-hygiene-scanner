...
def run_cli(args, cwd=None):
    ...
    proc = subprocess.run([PYTHON, "-m", "rag_hygiene_scan.cli", *args],
...
def test_json_output_and_summary(tmp_path: Path):
    ...
    code, out, err = run_cli([str(tmp_path), "--format", "json", "--fail-on", "med", "--summary"])
...
def test_csv_output_to_file_and_exit_code(tmp_path: Path):
    ...
    code, out, err = run_cli([str(tmp_path), "--format", "csv", "-o", str(out_path), "--fail-on", "high"])
    ...
    assert any("HTML003" in line or "HTML001" in line or "HTML002" in line for line in content[1:])
